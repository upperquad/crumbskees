...
const Board = props => {
  ...
  const renderPlayerMeta = (player, itemsForThisRound, power, index) => {
    const placeholderLength = Math.max(20 - itemsForThisRound.length, 0)
    return (
      <div className={classNames(styles.playerMeta, styles[`playerMeta--${index + 1}`])}>
        ...
        <div className={styles.items}>
          ...
          {[...Array(placeholderLength)].map((e, i) => {
  ...
  const timerPercentage = Math.min(time / totalTime, 1)
  ...
  return (
    <div className={classNames(styles.board, { [styles.boardEntering]: transitionStatus === 'entering' })}>
      ...
      <div className={styles.timer}>
        <div className={styles.timerMaskLeft} style={{ width: `${timerPercentage * 100}%` }} />
        <div className={styles.timerMaskRight} style={{ width: `${(1 - timerPercentage) * 100}%` }} />
